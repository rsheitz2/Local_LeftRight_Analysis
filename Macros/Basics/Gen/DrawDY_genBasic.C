...
void DrawDY_genBasic(){
  ...
  TTree *t_AMDY = (TTree*)f_AMDY->Get("Event");
  
  TFile* f_JPsi =
    TFile::Open(path+"JPsi.root");
  TTree *t_JPsi = (TTree*)f_JPsi->Get("Event");
    
  TFile* f_psi =
    TFile::Open(path+"psi_minusW10_SP2.root");
  TTree *t_psi = (TTree*)f_psi->Get("Event");
  
  TFile* f_OC =
    TFile::Open(path+"OC.root");
  TTree *t_OC = (TTree*)f_OC->Get("Event");
