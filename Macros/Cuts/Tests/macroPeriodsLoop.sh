...
for per in ${period[@]}; do
    echo ""
    echo "Period ${per}  for ${Macro}"
    echo ""

    #macro changes
    ${cutsPath}/Scripts/changeMacro.sh ${cutsPath}/${macroBase} ${per} $Mtype $production $whichCuts
    
    #Execute
    root -l -b -q "${PWD}/${Macro}(1)" >> ${cutsPath}/${macroBaseName}_log.txt
    if [ $? != 0 ]; then
	echo "${PWD}/${Macro} did not execute well"
