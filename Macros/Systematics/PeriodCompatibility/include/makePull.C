...
void makePull(TString fileNameStart, TString fileNameMiddle,
  ...
  //TH1D* hPull_pull = new TH1D("hPull_pull", "hPull_pull", 14, -5, 5);
...
void FinalWAvgs(Double_t &wMean, Double_t &wMeanSig2,
  ...
  cout << "weighted systematic error/stat error = "
       << CalSysErrorPeriod(wMean, wSig2) << endl;
}
...
void SetUpPull(TH1D *h){
  ...
  //h->Fit("gaus", "LQ");
  h->Fit("gaus", "WLQ");
  h->GetFunction("gaus")->SetLineStyle(9);
  h->GetFunction("gaus")->SetLineWidth(4);
  FinalSetup(h);
}
