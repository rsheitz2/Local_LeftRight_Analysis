...
cp ${HOME}/genRej.C ${HOME}/genRej_tmp.C
...
for phiS in ${phiScut[@]}; do
	...
	${HOME}/Scripts/ChangeScripts/changeMacro.sh ${HOME}/genRej.C $phiS $A $additionalCuts $N_gen $alphaScale
	
	#Execute
	#root -l -q -b "${HOME}/generator.C" >> ${HOME}/generator_log.txt
	root -l -q -b "${HOME}/genRej.C" >> ${HOME}/genRej_log.txt
	if [ $? != 0 ]; then
	    #echo "generator.C did not execute well"
	    #mv ${HOME}/generator.C ${HOME}/generator.C.bak
	    #mv ${HOME}/generator_tmp.C ${HOME}/generator.C

	    echo "genRej.C did not execute well"
	    mv ${HOME}/genRej.C ${HOME}/genRej.C.bak
	    mv ${HOME}/genRej_tmp.C ${HOME}/genRej.C
	    exit 1
	fi
