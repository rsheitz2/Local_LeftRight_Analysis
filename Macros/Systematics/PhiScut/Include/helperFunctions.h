...
void DrawLine(TH1D *h, Double_t yval, TString orientation ="h"){
  TLine* li;
  if (orientation == "h"){//Horizontal line
    Double_t min_x = h->GetXaxis()->GetXmin();	
    Double_t max_x = h->GetXaxis()->GetXmax();	
    
    li = new TLine(min_x, yval, max_x, yval);
  }
  else if (orientation == "v"){//Horizontal line
    Double_t min_y = h->GetMinimum();
    Double_t max_y = h->GetMaximum();
    li = new TLine(yval, min_y, yval, max_y);
  }
...
void DrawLine(TH1D *h, Double_t yval, Int_t color, TString orientation ="h"){
  TLine* li;
  if (orientation == "h"){//Horizontal line
    Double_t min_x = h->GetXaxis()->GetXmin();	
    Double_t max_x = h->GetXaxis()->GetXmax();	
