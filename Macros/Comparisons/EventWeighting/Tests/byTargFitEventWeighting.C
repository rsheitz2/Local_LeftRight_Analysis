...
/TGeant/Local_LeftRight_Analysis/Macros/Comparisons/EventWeighting/Data";
  ...
  Double_t A_int, eA_int;
  gMinuit->GetParameter(5, A_int, eA_int);
  Double_t A_int_corr, eA_int_corr;
...
Int_t DrawContour(TCanvas *c, TString whichBin="int"){
  ...
  for (Int_t i=0; i<5; i++) {
    ...
    //gMinuit->SetErrorDef(1.5);
    TGraph *g_cont_2sig = (TGraph*)gMinuit->Contour(40, 5, i);
    ...
    TGraph *g_cont = (TGraph*)gMinuit->Contour(40, 5, i);
