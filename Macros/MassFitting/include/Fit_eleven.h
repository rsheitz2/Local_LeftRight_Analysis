...
Double_t eleven_CrystalBall(Double_t *x, Double_t *par){
  ...
  if (arg > -par[3] ) return par[0]*TMath::Exp(-0.5*arg*arg);
  else return par[0]*A*TMath::Power((B - arg), -par[4]);
...
Double_t Fit_eleven_UpS(Double_t *x, Double_t *par){
  Double_t xShift = x[0]-par[12];
  Double_t ratioPsi = Get_eleven_Ratio("UpS");

  Double_t par_JPsi[] = {par[0], par[1], par[2], par[3], par[4]};
  Double_t JPsi = eleven_CrystalBall(x, par_JPsi);
  
  Double_t par_psi[] =
    {par[5], par[1]*ratioPsi, par[2]*ratioPsi, par[6], par[7]};
  Double_t psi = eleven_CrystalBall(x, par_psi);
...
Double_t Fit_eleven_DownS(Double_t *x, Double_t *par){
  Double_t xShift = x[0]-par[12];
  Double_t ratioPsi = Get_eleven_Ratio("DownS");
...
Double_t Fit_eleven_scan(Double_t *x, Double_t *par){
...
void Paras_eleven(TH1D *h, TF1 *fitFunc, Int_t nPar,
		 Double_t Mmin, Double_t Mmax, Bool_t hIsUpS){
  //Normal parameter setup
  Int_t ipar =0;
  Double_t ratioPsi = (hIsUpS) ?
    Get_eleven_Ratio("UpS") : Get_eleven_Ratio("DownS");
  ...
  //fitFunc->SetParLimits(2, 0, 0.22); ipar++;

  fitFunc->SetParLimits(3, 1.2, 5.0); ipar++;
  fitFunc->SetParLimits(4, 0.9, 30.0); ipar++;
  
  fitFunc->SetParLimits(5, A_psi/(factor), A_psi*factor); ipar++;//psi
  fitFunc->SetParLimits(6, 0.5, 2.0); ipar++;
  fitFunc->SetParLimits(7, 0.5, 10.0); ipar++;
  
  fitFunc->SetParLimits(8, 0.0, A_Bg*factor); ipar++;//CombBg
  //fitFunc->SetParLimits(9, -7.0, -2.0); ipar++;
  fitFunc->SetParLimits(10, 0.0, A_DY*factor); ipar++;//DY
  //fitFunc->SetParLimits(11, -2.0, 0.0); ipar++;
  
  fitFunc->SetParLimits(12, Mmin, Mmin); ipar++;//Mmin
  ...
  fitFunc->SetParLimits(ipar, 1.0, 3.0); ipar++;
  fitFunc->SetParLimits(ipar, 1.0, 30.0); ipar++;
  
  fitFunc->SetParLimits(ipar, A_psi/(factor), A_psi*factor); ipar++;//psi
  fitFunc->SetParLimits(ipar, 1.0, 4.0); ipar++;
  fitFunc->SetParLimits(ipar, 0.5, 10.0); ipar++;
  ...
  fitFunc->SetParLimits(ipar, 1.0, 10.0); ipar++;
...
TF1* SetupFunc_eleven(TH1D *h, Bool_t hIsUpS, TF1 *fitFunc,
		     Double_t Mmin, Double_t Mmax, Int_t *nPar){
  //Setup fit function
  *nPar =13;

  if (hIsUpS) fitFunc = new TF1("fitFunc", Fit_eleven_UpS, Mmin, Mmax, *nPar);
  else fitFunc = new TF1("fitFunc", Fit_eleven_DownS, Mmin, Mmax, *nPar);

  //Setup intial parameters of fit and parameter constraints
  Paras_eleven(h, fitFunc, *nPar, Mmin, Mmax, hIsUpS); //Newer more accurate pars

  return fitFunc;
}


TF1* SetupFunc_eleven(TH1D *h, TF1 *fitFunc, Double_t Mmin, Double_t Mmax,
		     Int_t *nPar, Double_t psiMW){
  //Setup fit function for scanning psiMW
  *nPar =14;

  fitFunc = new TF1("fitFunc", Fit_eleven_scan, Mmin, Mmax, *nPar);

  //Setup intial parameters of fit and parameter constraints
  Paras_eleven(h, fitFunc, *nPar, Mmin, Mmax, psiMW);

  return fitFunc;
}


void ProcessPars_eleven(TF1 *fitFunc, Double_t *processPars,Double_t *LR_cov,
  ...
  Double_t psiMW = (hIsUpS) ? Get_eleven_Ratio("UpS") : Get_eleven_Ratio("DownS");
  ...
  if (process=="JPsi"){
    ...
    cout << "Process not defined well in Fit_eleven" << endl;
    exit(EXIT_FAILURE);
  }
}
...
void ProcessPars_eleven(TF1 *fitFunc, Double_t *processPars,Double_t *LR_cov,
  ...
  ProcessPars_eleven(fitFunc, processPars, LR_cov, cov, process,nPars,hIsUpS);
...
void ProcessPars_eleven(TF1 *fitFunc, Double_t *processPars,
  ...
  Double_t psiMW;
  if (hIsUpS) psiMW = Get_eleven_Ratio("UpS");
  else psiMW = Get_eleven_Ratio("DownS");
...
void ProcessPars_eleven(TF1 *fitFunc, vector<vector<Double_t> > &processPars,
...
void ProcessPhysicsPars_eleven(TF1 *fitFunc, Double_t *processPars,
...
TF1* ComponentFuncts_eleven(Double_t *pars, Double_t Mmin, Double_t Mmax,
			   TString process, Bool_t hIsUpS){
  Double_t psiMW = (hIsUpS) ? Get_eleven_Ratio("UpS") : Get_eleven_Ratio("DownS");
  
  TF1 *JPsi = new TF1("f_JPsi", eleven_CrystalBall, Mmin, Mmax, 5);
  JPsi->SetParameters(pars[0], pars[1], pars[2], pars[3], pars[4]);
  
  TF1 *psi = new TF1("f_psi", eleven_CrystalBall, Mmin, Mmax, 5);
  psi->SetParameters(pars[5], pars[1]*psiMW, pars[2]*psiMW, pars[6], pars[7]);
  ...
  if (process == "JPsi") {
    ...
    cout << "Wrong input process Fit_eleven" << endl;
    exit(EXIT_FAILURE);
  }
}


void IntegrateLR_eleven(TF1 *f, Double_t binWidth,
...
void IntegrateLR_eleven(TF1 *f, Double_t *pars, Double_t *LR_cov,
...
void IntegrateLR_eleven(TF1 *f, Double_t *pars, Double_t *LR_cov,
  ...
  Double_t psiMW;
  if (process == "psi") {
    psiMW = (hIsUpS) ? Get_eleven_Ratio("UpS"):Get_eleven_Ratio("DownS");
    Mass *= psiMW;
  }
  ...
  if (process =="JPsi"){
    ...
    cout << "Process not defined for this IntegrateLR_eleven" << endl;
  }
}


void IntegrateLRpercent_eleven(TF1 *f, Double_t *pars, Double_t *LR_cov,
  ...
  IntegrateLR_eleven(f, pars, LR_cov, binWidth, LR_Mmin, LR_Mmax, LR, e_LR,
		    Mmin, Mmax, hIsUpS, process);
}


void IntegrateLRpercent_eleven(TF1 *f, Double_t *pars, Double_t binWidth,
  ...
  IntegrateLR_eleven(f, binWidth, LR_Mmin, LR_Mmax, LR, e_LR);
  }//*/
