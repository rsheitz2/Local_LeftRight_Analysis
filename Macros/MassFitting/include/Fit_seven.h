...
void ProcessPars_seven(TF1 *fitFunc, Double_t *processPars,
  ...
  if (process=="JPsi"){
    ...
    processPars[0] = pars[4]; //Amplitude
    processPars[1] = pars[5]; //Slope

    e_processPars[0] = e_pars[4]; //Amplitude
    e_processPars[1] = e_pars[5]; //Slope
  }
  else{
    cout << "Process not defined well in Fit_seven" << endl;
    exit(EXIT_FAILURE);
