void ByTarg_ByProcess_genIntegrated(){
  ...
  TString path = "/Users/robertheitz/Documents/Research/DrellYan/Analysis/TGeant/Presents/April17/Data/Gen/";
  TFile* f_AMDY_int =
    TFile::Open(path+"AMDY_gen_byTarget_1bin.root");
  TGraphErrors* g_AMDY_int[nGr];
  
  TFile* f_JPsi_int =
    TFile::Open(path+"JPsi_gen_byTarget_1bin.root");
  TGraphErrors* g_JPsi_int[nGr];

  TFile* f_psi_int =
    TFile::Open(path+"psi_minusW10_SP2_gen_byTarget_1bin.root");
  TGraphErrors* g_psi_int[nGr];
  
  TFile* f_OC_int =
    TFile::Open(path+"OC_gen_byTarget_1bin.root");
  ...
  TGraphErrors **g_Process[] = {g_AMDY_int, g_JPsi_int, g_psi_int, g_OC_int};
  ...
  Double_t yMax = 0.004;
  g_Process[pDraw][0]->GetXaxis()->SetLimits(0, 0.5);
