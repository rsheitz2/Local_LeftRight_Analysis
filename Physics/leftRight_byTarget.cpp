...
int main(int argc, char **argv){
  ...
  Int_t iflag=0, aflag=0, binFlag=0, Mflag=0, Dflag=0, Cflag=0, Hflag=0;
  Int_t c;
  TString fname = "", outFile = "", leftrightChoice="", trig="";
  TString binFile = "", massRange="";
  Double_t M_min=0.0, M_max=12.0;
  
  while ((c = getopt (argc, argv, "Pwf:Q:S:T:i:a:b:M:DCH")) != -1) {
    switch (c) {
    case 'w':
      wflag = 1;
      break;
  ...
  if (Cflag) {
    for (Int_t ev=0; ev<tree_entries; ev++) {
      ...
      if (targetPosition == 0) nUpStream++;
      else if (targetPosition == 1) nDownStream++;
  ...
  for (Int_t ev=0; ev<tree_entries; ev++) {
	...
	cout << "Trigger mask set to: " << trig << " only" << endl;
      ...
      if (Cflag){
	cout << "PolUp and PolDown switched!!" << endl;
      }
	...
	if ( (!Cflag&&Spin_0>0) || (Cflag&&stopUpStream<nUpStream/2.0) ){//Polarized Up
	...
	else if ( (!Cflag&&Spin_0<0) || (Cflag&&stopUpStream>nUpStream/2.0) ){//Polarized Down
	...
	if ( (!Cflag&&Spin_0>0) || (Cflag&&stopDownStream<nDownStream/2.0) ){//Polarized Up
	...
	else if ( (!Cflag&&Spin_0<0) || (Cflag&&stopDownStream>nDownStream/2.0) ){//Polarized Down
    ...
    if (!Cflag && binFlag){//Average Polarization
      // {{{
