...
int main(int argc, char **argv){
  ...
  TH1D *hPhi1[nBins], *hPhi2[nBins];
  for (Int_t i=0; i<nBins; i++) {
    hPhi1[i] = new TH1D(Form("hPhi1_%f", M_bounds[i]), Form("hPhi1_%f", M_bounds[i]),
			//hPhi1[i] = new TH1D(Form("hPhi1_%i", i), Form("hPhi1_%i", i),
			100, -TMath::Pi(), TMath::Pi() );
    //100, 0, 0.21);//Theta
			//200, 0, 200);//qP
    hPhi2[i] = new TH1D(Form("hPhi2_%f", M_bounds[i]), Form("hPhi2_%f", M_bounds[i]),
    //hPhi2[i] = new TH1D(Form("hPhi2_%i", i), Form("hPhi2_%i", i),
			100, -TMath::Pi(), TMath::Pi() );
    //100, 0, 0.21);//Theta
    //200, 0, 200);//qP
  }
  ...
  for (Int_t ev=0; ev<tree_entries; ev++) {
    ...
    //if (trigMask != 65792) continue; //Only Last-Last
    //if (trigMask != 65540) continue; //Only Last-Outer
    //if (trigMask != 65796) continue; //Only Last-Outer or Last-Last only
    //if (trigMask !=65792 && trigMask != 65796) continue; //Only Last-Outer or Last-Last only or only Last last
    //Choose Left/Right
    // {{{
    ...
    //if (theta_traj2 < 0.06) continue;
    //if (theta_traj2 > 0.06) continue;
    //if (Mmumu > 3.29 || Mmumu < 3.11) continue;
    
    BinDataFill(hPhi1, phi_traj1, nBins, Mmumu, M_bounds);
    BinDataFill(hPhi2, phi_traj2, nBins, Mmumu, M_bounds);

    //BinDataFill(hPhi1, theta_traj1, nBins, Mmumu, M_bounds);
    //BinDataFill(hPhi2, theta_traj2, nBins, Mmumu, M_bounds);

    //BinDataFill(hPhi1, qP_traj1, nBins, Mmumu, M_bounds);
    //BinDataFill(hPhi2, -1.0*qP_traj2, nBins, Mmumu, M_bounds);
  ...
  */
  ...
  cPhi->Divide(2, 5);
  TCanvas* cPhi_more = new TCanvas();
  cPhi_more->Divide(2, 5);
  for (Int_t i=0; i<nBins; i++) {
    
    (i<10) ? cPhi->cd(i+1) : cPhi_more->cd(i-9);
    
    hPhi1[i]->Draw();
    hPhi2[i]->Draw("sames");
    hPhi2[i]->SetLineColor(kRed);

    
    }//*/

  TFile *f_out = new TFile("JPsi_phi.root", "RECREATE");
  for (Int_t i=0; i<nBins; i++) {
    hPhi1[i]->Write();
    hPhi2[i]->Write();
  }
  ...
  /*TCanvas* cPhi1 = new TCanvas();
  TCanvas* cPhi2 = new TCanvas();
  for (Int_t i=0, icolor=1; i<nBins; i++, icolor++) {
    cPhi1->cd();
    (i==0) ? hPhi1[i]->Draw() : hPhi1[i]->Draw("same");

    cPhi2->cd();
    (i==0) ? hPhi2[i]->Draw() : hPhi2[i]->Draw("same");

    
    if (icolor==5) icolor++;
    else if (icolor==10) icolor=1;

    hPhi1[i]->SetLineColor(icolor);
    hPhi2[i]->SetLineColor(icolor);
